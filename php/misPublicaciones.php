...
//------------------------------------------------
$query="SELECT * FROM PUBLICACION WHERE fk_pu='$id_session'";
$publicaciones=mysql_query($query,$connection) or die(json_encode(array('type'=> 'error','text'=>"Error al hacer consulta" .mysql_error())));
...
$i=0;
  ...
  {
    $jsondata['type']='suss';
    while($row=mysql_fetch_assoc($publicaciones))
    {
      //$jsondata['urlFoto']=$rowfoto['url_foto'];
      $jsondata["publicacion"][$i]=$row;
      $i++;
