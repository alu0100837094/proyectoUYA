...
$comprobar = mysql_query("SELECT * FROM MyGuests WHERE email='$emailR'", $connection) or die('Consulta fallida :' .mysql_error());
$rows = mysql_num_rows($comprobar);
if ($rows == 1) {
echo "false"; // Falso, ya hay un usuario con ese email
} else {
//echo"Contraseña y/o email incorrectos";
$ingresar = mysql_query("INSERT INTO MyGuests (firstname,lastname,email,password) VALUES ('$nombre','$apellidos','$emailR','$passwordR')",$connection) or die('Ingreso fallido : ' . mysql_error());
echo "true";
